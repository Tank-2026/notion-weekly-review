...
# 3. 生成复盘内容
content = f"""【Workbuddy自动生成 - {now.strftime('%Y-%m-%d %H:%M')} UTC】

📊 本周任务结构分析：
- 完成任务总数：{total_done}条
- 推进型任务（挂了项目的）：{push}条（{push_pct}%）
- 维护型任务（未挂项目的）：{maintain}条（{100-push_pct}%）
{"⚠️ 推进型<30%：本周陷入简单重复，下周需拉KR任务上来" if push_pct < 30 else "✓ 推进型占比正常"}
